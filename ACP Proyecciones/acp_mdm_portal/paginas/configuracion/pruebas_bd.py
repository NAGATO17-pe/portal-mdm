...
def render():
    ...
    st.markdown('<h1 class="main-header">🛠️ Pruebas de Base de Datos</h1>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Entorno para consultar y evaluar los datos alojados en SQL Server de manera amigable.</p>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if st.button("▶️ Ejecutar Consulta Visual", type="primary", key="btn_visual", use_container_width=True):
    ...
    with tab2:
        ...
        with btn_col1:
            if st.button("🚀 Ejecutar T-SQL", type="primary", key="btn_tsql", use_container_width=True):
                query_a_ejecutar = query_libre
        with btn_col2:
            if st.button("🧹 Limpiar Texto", use_container_width=True):
                st.rerun()
