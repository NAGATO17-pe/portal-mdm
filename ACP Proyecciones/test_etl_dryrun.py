...
def analizar_archivo_bronce(
    ...
    try:
        ...
        for col in columnas_fecha:
            ...
            no_parseables = muestra[
                pd.to_datetime(muestra, errors="coerce", dayfirst=True).isna()
            ]
            if len(no_parseables) > 0:
                resultado.advertencias.append(
                    f"'{col}' tiene fechas no parseables: {no_parseables.tolist()[:3]}"
                )
...
def construir_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--carpeta_bronce",
        type=Path,
        help="Ruta a la carpeta con los Excel fuente de Bronce",
        default=None,
    )
    parser.add_argument(
        "--solo_utils",
    ...
    parser.add_argument(
        "--tabla",
        type=str,
        help="Analizar solo un archivo específico (ej: Conteo_Fruta)",
        default=None,
    )
    return parser
...
def main() -> None:
    ...
    if not args.solo_utils:
        if args.carpeta_bronce is None:
            print("\n  ℹ  --carpeta_bronce no especificada. Solo se prueban utils.\n")
        elif not args.carpeta_bronce.exists():
            print(f"\n  ❌  La carpeta no existe: {args.carpeta_bronce}\n")
            sys.exit(1)
        else:
            archivos_xlsx = sorted(args.carpeta_bronce.glob("*.xlsx"))

            if args.tabla:
                archivos_xlsx = [
                    a for a in archivos_xlsx
                    if args.tabla.lower() in a.stem.lower()
                ]

            if not archivos_xlsx:
                print(f"\n  ⚠  Sin archivos .xlsx en: {args.carpeta_bronce}\n")
            else:
                print(f"\n  Escaneando {len(archivos_xlsx)} archivos...\n")
