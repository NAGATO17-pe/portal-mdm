...
from silver.facts import fact_tasa_crecimiento_brotes as modulo


def test_fact_tasa_crecimiento_brotes_usa_misma_transaccion_para_side_effects(monkeypatch):
    class _Conexion:
        def __init__(self):
        ...
        def execute(self, sentencia, payload=None):
            texto = str(sentencia)
            if 'INSERT INTO Silver.Fact_Tasa_Crecimiento_Brotes' in texto:
    ...
    monkeypatch.setattr(modulo, '_validar_layout_migrado', lambda _engine: 'ID_Tasa_Crecimiento')
    monkeypatch.setattr(
        modulo,
        '_leer_bronce',
        lambda _engine, _columna_id: pd.DataFrame([
            {
                ...
                'Cama_Raw': '10',
                'Condicion_Raw': 'Buena',
                'Estado_Vegetativo_Raw': 'Activo',
                'Variedad_Raw': 'Biloxi',
                'Tipo_Tallo_Raw': 'Principal',
                'Ensayo_Raw': 'ENS-1',
                'Medida_Raw': '2.5',
                'Fecha_Poda_Aux_Raw': '2026-04-01',
                'Campana_Raw': '2026',
                'Observacion_Raw': 'ok',
                'Tipo_Evaluacion_Raw': 'Semanal',
                'Codigo_Origen_Raw': 'COD-1',
            },
            {
    ...
    resumen = modulo.cargar_fact_tasa_crecimiento_brotes(engine)
